...
function! s:GroupingAlgorithmOriginal(targets, keys)
    if g:EasyMotion_num > 9
        let groups = {}
        let targets1 = a:targets
        let l:ri1 = 0
        for target in targets1
            let l:ri = index(targets1, target) + 1
            if l:ri < 10
                let l:ri2 = l:ri
            else
                let l:ri1 = l:ri[0]
        ...
        while 1
            ...
            if EasyMotion#helper#is_folded(pos[0])
                if search_direction ==# 'b'
                    ...
                    "comment the line out: then no hint gabs in folded lines
