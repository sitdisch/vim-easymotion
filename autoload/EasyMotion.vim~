...
function! EasyMotion#Repeat(visualmode,direction) " {{{
    ...
    let g:EasyMotion_repeat_direction = s:previous.direction
    let s:previous.regexp = l:regexp
    let s:previous.direction = l:direction
    let s:previous['regexp_without_search_fFtT'] = l:regexp_without_search_fFtT
    return s:EasyMotion_is_cancelled
...
function! EasyMotion#NextPrevious(visualmode, direction) " {{{
    ...
    let g:EasyMotion_repeat_direction = l:direction
...
function! s:GetChar(...) abort "{{{
    if g:EasyMotion_showdelay > 0
      if exists("g:CustomMacroRun")
          let g:sleep_interruption_key = ''
          sleep 100m
          return g:EasyMotion_target
      elseif g:EasyMotion_showdelay == 2
          if exists("g:last_operator") && g:last_operator =~# '[yd]' || g:repeat_mode == 1
              let g:sleep_interruption_key = ''
              sleep 150m
          else
              let g:sleep_interruption_key = GetKeyWithTimeout(150)
          endif
          return ''
      else
          if exists("g:last_operator") && g:last_operator =~# '[yd]' || g:repeat_mode == 1
              ...
              let g:sleep_interruption_key = GetKeyWithTimeout(100)
          endif
          return g:EasyMotion_target
      endif
    else
...
" }}}
" Customized function for flashing counts 
...
function! s:EasyMotion(regexp, direction, visualmode, is_inclusive, ...) " {{{
    ...
    try
        ...
        if g:EasyMotion_searchfoldedlines == 1
            let l:ri1 =  0
            while 1
                " Reached end of search range
                if (pos == [0, 0])
                    let g:EasyMotion_flash_adapt_num = l:ri1
                    break
                elseif (l:ri1 > 99)
                    break
                endif
        ...
        if g:EasyMotion_do_shade && targets_len != 1 && s:flag.dot_repeat != 1
            " Backward shading doesn't work, if the first line is folded, 
            " so shade always everything
